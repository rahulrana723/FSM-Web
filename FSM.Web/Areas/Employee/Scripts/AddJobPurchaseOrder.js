...
var Purchaseorderjob = {
    ...
    CheckISPurchaseavailable :function() {
        ...
        var CurrentPurchaseNo = $("#PurchaseOrderNo").val();
    $.ajax({
        ...
        success: function (dataSuccess) {
            if (dataSuccess.result != CurrentPurchaseNo) {
                ...
                $("#PurchaseOrderNo").val(purchaseno);
                Purchaseorderjob.isvalidpurchase = "false";
            }
            else {
                
                Purchaseorderjob.isvalidpurchase = "true";
            }
...
$(document).on('click', '.btnAddstockitem', function () {
    Label = $(".ddlstock").val();
    Stockid = "";
    purchaseItem = $(".ddlstock").val();
    unitMeasure = $("#UnitOfMeasure").val();
    Quantity = $("#Quantity").val();
    if (Quantity == '0' || Quantity == 'undefined' || Quantity == "") {
        var flag = 1;
    }
    if (purchaseItem == '0' || purchaseItem == '' || purchaseItem == 'undefined') {
        flag = 1;
    }
    if (flag == 1) {
    ...
    if (Purchaseorderjob.RowNotoEdit == 0) {
        ...
        trtoEdit.children('td')[1].textContent = purchaseItem;
...
$(document).on('click', '.btnEditstock', function () {
    ...
    $(".ddlstock").val(PurchaseItem.toString().toLowerCase());
...
$(document).on('click', '.btnaddjobpurchaseorder', function () {
    Purchaseorderjob.CheckISPurchaseavailable();
    if (Purchaseorderjob.isvalidpurchase = "true") {
...
$(document).on('click', '.btnaddjobpurchaseorderInvoiceJobid', function () {
        ...
        if (rowCount > 1) {
            ...
            if (supplier != '0' && supplier != 'undefined' && supplier != "" && supplier != null) {
                ...
                $.ajax({
                    ...
                    async: false,
                    success: function (data) {
                        window.location.href = common.SitePath + '/Employee/Invoice/ViewJobsPurchaseOrder?JobId=' + realJobId + '&InvoiceId=' + FSM.InvoiceId;
                        //  $("#jobpurchaseorder").modal('hide');
                    },
                    error: function () {
                    }
