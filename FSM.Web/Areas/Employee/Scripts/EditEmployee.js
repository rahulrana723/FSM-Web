...
$('#SignatureDoc').change(function () {
    var fileUpload = $("#SignatureDoc").get(0);
...
$('#ProfileDoc').change(function() {
    var fileUpload = $("#ProfileDoc").get(0);
    var id = fileUpload.id;
    var files = fileUpload.files;
...
function DisplayImages(fileData, id) {
    $.ajax({
        ...
        success: function (result) {
            ...
            if (id == "SignatureDoc") {
                $("#list_files").append("<tr><td>" + "Signature Picture" + "</td><td>" + fname + "</td><td>" + "<img src='" + FSM.EditEmployeeROOT + fname + "' class='imgSrc' width='74' height='74'/>" + "</td></tr>");
                //$('#divSignaturePic').empty();
                //$('#divSignaturePic').append("<img src='" + FSM.EditEmployeeROOT + fname + "' width='74' height='74' />");
                //$('#SignaturePictureTemp').val(fname);
            }
               
            else if (id == "ProfileDoc") {
                //$('#divProfilePic').empty();
                $("#list_files").append("<tr><td>" + "Profile Picture" + "</td><td>" + fname + "</td><td>" + "<img src='" + FSM.EditEmployeeROOT + fname + "' class='imgSrc' width='74' height='74'/>" + "</td></tr>");
            }
            else if (id == "DrivingLicense") {
                //$('#divDrivingLicenseImg').empty();
                $("#list_files").append("<tr><td>" + "Driving License" + "</td><td>" + fname + "</td><td>" + "<img src='" + FSM.EditEmployeeROOT + fname + "' class='imgSrc' width='74' height='74'/>" + "</td></tr>");
            }
            else if (id == "BankDetail") {
                //$('#divBankDetailDoc').empty();
                $("#list_files").append("<tr><td>" + "Bank Detail" + "</td><td>" + fname + "</td><td>" + "<img src='" + FSM.EditEmployeeROOT + fname + "' class='imgSrc' width='74' height='74'/>" + "</td></tr>");
            }
            else if (id == "Insurance") {
                //$('#divInsurance').empty();
                $("#list_files").append("<tr><td>" + "Insurance" + "</td><td>" + fname + "</td><td>" + "<img src='" + FSM.EditEmployeeROOT + fname + "' class='imgSrc' width='74' height='74'/>" + "</td></tr>");
            }
        },
...
$(document).on('blur', '#Username',      //validate o duplicate username
      function () {
          var EmpUserName = $(this).val();
          var currentUser= FSM.CurrentUserName;
          if (EmpUserName != "") {
              $.ajax({
                  url: common.SitePath + "/Employee/Employee/CheckUserNameExist",
                  data: { UserName: EmpUserName, CurrentUserName: currentUser },
                  type: 'Get',
                  async: false,
...
$(document).on('blur', '.EmpEmail', function () {                                   //validate on duplicate email
    ...
    var Email = $(this).val();
    var cuurentEmail = FSM.CurrentEmail;
    if (Email != "") {
        ...
        $.ajax({
            ...
            data: { EmpEmail: Email, CurrentEmail: cuurentEmail },
            type: 'Get',
            async: false,
            success: function (data) {
                if (data === 0) {
...
$(document).on("click", "#list_files tbody tr", function () {  //click table row image show new tab
    var imageSource = $(this).find('.imgSrc').attr('src');
