...
var InvoiceEmail = {
    SendMailWithAttachment: function () {
        ...
        var to = [];
        var emails = "";
        var collection = $("#select-tools").val();
        
        to = collection.split(';');
        var ccollection = $("#select-cc").val();
        var cc = ccollection.split(';');
        var bccollection = $("#select-bcc").val();
        var bcc = bccollection.split(';');
        ...
        if (bcc != "") {
            var reg = /^([A-Za-z0-9_\-\.])+\@([A-Za-z0-9_\-\.])+\.([A-Za-z]{2,4})$/;
            for (var i = 0, l = bcc.length; i < l; i++) {
                var value = bcc[i];
                ...
                if (reg.test(value) == true) {
                    ...
                    $("#BCCError").text("*Please enter valid email address");
                    return false;
                }
            }
        }
        ...
        $.ajax({
            ...
            success: function (data) {
                if (data.responseText != "") {
                    ...
                    if (data.invoiceAlreadySent == true) {
                        window.location.href = common.SitePath + "/Employee/Invoice/SaveInvoiceInfo/" + invoiceID 
                    }
                }
            },
            error: function () {
