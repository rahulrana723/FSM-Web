...
$(document).on('click', '.up', function (event) {
    ...
    InvoiceData.CheckFirstandLast();
    //disbled first up arrow
});
...
var InvoiceData = {
    ...
    CalculteJCLITemAmounts: function () {
       
        var Quantity = 0;
        var sum = 0;
        var price = $(this).parent().parent().find('.jclprice').html()
        var Total = 0;
        if (price === NaN || price === undefined || price === null) {
            Total = 0;
        }
        else {
    ...
    CalculatePercentagePayInvoice: function () {
        var amountPaid = $("#amountPaid").val();
        var advancepaid = 0;

        var Gtotal = $(".TotalWithGSTLbl").text();
        if (amountPaid != '' && amountPaid != null && amountPaid != 'undefined') {
            //check for invoice
            var balance = $(".divQuoteamountbal").text();
            if (balance != '' && balance != null && balance != 'undefined') {
                ...
                var balanceDue = (balance) - ((balance * amountPaid) / 100);
        ...
        //    var balance = $(".divQuoteamountbal").text();
        //    if (balance != '' && balance != null && balance != 'undefined') {
        //        advancepaid = parseFloat(balance * amountPaid) / 100;
        //        $(".amountPay").text(advancepaid);
        //        var balanceDue = (balance) - ((balance * amountPaid) / 100);
        //        $(".balanceDue").text(balanceDue);
        //    }
        ...
        $("#createInvoiceViewModel_Price").val(MainBalance);
    ...
    CheckFirstandLast: function () {
        ...
        var $table = $('.up').parent().parent().parent().parent().parent();
        $table.find('tbody').find('tr:first').find('.up').attr('disabled', 'disabled');       //disbled first up arrow
        $table.find('tbody').find('tr:last').find('.down').attr('disabled', 'disabled');
...
$(document).ready(function () {
    ...
    var otrwjobnoteId = $('#createInvoiceViewModel_OTRWNotes').prev("div").prop("id");
    ...
    InvoiceData.CalculteJCLITemAmounts();
    // $("#amountPaid").val("");
    //Get Amount Paid And Show Total Price After AMount Paid Selection

    InvoiceData.CalculatePercentagePayInvoice();
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            row = row + "<td class='Mappingid' style='display:none'></td>\
                            ...
                            <td class='jclprice'><input type='text' maxlength='10' class='edtprice form-control text-box single-line'></input></td>\
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                for (var i = 0; i < JclInfo.length; i++) {
                    ...
                    parenttr.find(".jclprice").html("<input type='text' maxlength='10' class='edtprice form-control text-box single-line' value=" + JclInfo[i]["Price"] + "> </input>");
...
$(document).on("change", "#amountPaid", function () {
    var valuepercentage = $("#amountPaid").val();

    if (valuepercentage != null && valuepercentage != "" && valuepercentage != 'undefined') {
        $("#divpaymentshowhide").css("display", "block");
        InvoiceData.CalculteJCLITemAmounts();
        //Get Amount Paid And Show Total Price After AMount Paid Selection
        InvoiceData.CalculatePercentagePayInvoice();
    }
    ...
    if (itemcount > 1) {
        ...
        $(".InvoiceJCL_table tr").each(function (index) {
            if (index != 0) {
                ...
                var TotalPrice = $(this.cells[14]).text();
            }
        });
...
$(document).on('keyup', '.edtPayAmount', function () {
    ...
    sum = parseFloat(sum).toFixed(2);
    $(".TotalAmountPAid").text(sum);
    var balanceamt = parseFloat(MainBalance) - sum;
    balanceamt = balanceamt.toFixed(2);
    $(".owingbalanceAmt").text(balanceamt);
});

/*****************************************syncing invoice data*****************************************/
$(document).on('click', '.btnsyncinvoice', function () {

    var id = $(this).attr("data-val");
    $(".btnsyncinvoice").text("Syncing Invoice...");
    $.ajax({
        //  url: common.SitePath + "/Employee/Invoice/MyobSync",
        url: common.SitePath + "/Employee/Invoice/Syncmyob",
        data: { Id: id },
        type: 'Get',
        async: false,
        success: function (data) {
            debugger;
          //  Filefor FSM Company
           // window.location.href = "https://secure.myob.com/oauth2/account/authorize?client_id=jgnsyvj7brdpw7kb4ftpz3rk&redirect_uri=http://www.srag-portal.com/Employee/Invoice/Syncmyob&response_type=code&scope=CompanyFile";
            //USed for the live file 
             //window.location.href = "https://secure.myob.com/oauth2/account/authorize?client_id=enpstq27p2t9fq32aj4bkfx9&redirect_uri=http://localhost:54245/Employee/Invoice/Syncmyob&response_type=code&scope=CompanyFile";
            window.location.href = "https://secure.myob.com/oauth2/account/authorize?client_id=enpstq27p2t9fq32aj4bkfx9&redirect_uri=http://www.srag-portal.com/Employee/Invoice/Syncmyob&response_type=code&scope=CompanyFile";
        },
        error: function () {
            //  Filefor FSM Company
          //  window.location.href = "https://secure.myob.com/oauth2/account/authorize?client_id=jgnsyvj7brdpw7kb4ftpz3rk&redirect_uri=http://www.srag-portal.com/Employee/Invoice/Syncmyob&response_type=code&scope=CompanyFile";
