...
$(document).on('click', '.customer-siteName', function () {
    // getting customer  id
    var id = $(this).siblings(":first").text();
    //getting site id
    var siteId = $(this).closest("tr").find('td:eq(4)').text();
    localStorage.setItem("siteID", siteId);   //value get in manage customer site
    window.location = FSM.EditCustomer + "?id=" + id + "&activetab=Customer Sites&hassitecount=yes";
});
